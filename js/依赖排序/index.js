...
// 会有一定的要求，比如 a 依赖了 b，那打包后的文件，b 需要排在 a 的前面
// 每一个依赖被加载后都会被立刻执行，那么如果要争取加载一个依赖，
// 则其子依赖都应该优先被加载
// 每一个依赖不希望在钱多出现冗余的情况，若依赖出现多版本的情况，
// 则默认使用更新的版本，比如已知项目依赖结构为（其中 @ 后面的为依赖版本号）：
...
// 输出分析：
// 为了让 a 加载后可以争取执行，则必须先加载 d 和 c，b 的加载同理，
// 又因为在整个依赖关系下，c 的最新版本为 0.2.0 于是有了如上的输出结果。
